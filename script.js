...
function displayFavorites() {
    ...
    favoritesList.innerHTML = ''; // Vider la liste avant de la remplir
...
document.getElementById('addToFavorites').addEventListener('click', () => {
    const currentCity = cityAndCountry.textContent.split(', ')[0];
    if (currentCity) {
...
window.onload = () => {
    textInput.value = '';
    executeProgram(randomCities[Math.floor(Math.random() * randomCities.length)]);
    displayFavorites();
};
